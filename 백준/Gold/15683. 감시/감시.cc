...
int n, m;
void SpreadCCTV(vector<vector<int>>& room, pair<int, int> cur_dir, int y,int x)
{
...
struct CCTV
{
	...
	void ApplyRotate(const int i, vector<vector<int>>& room) {
		// 90도 회전 x,y -> y,-x
...
vector<CCTV> cctvs;
int ans = 1 << 20;
void GO(const int cur_cctv,vector<vector<int>>& room)
{
